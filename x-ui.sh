...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/kooroshmoradi/CYRUS-X/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_menu() {
    ...
    wget -O /usr/bin/x-ui https://raw.githubusercontent.com/kooroshmoradi/CYRUS-X/main/x-ui.sh
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/kooroshmoradi/CYRUS-X/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/kooroshmoradi/CYRUS-X/master/install.sh)${plain}"
    echo ""
...
update_shell() {
    wget -O /usr/bin/x-ui -N https://github.com/kooroshmoradi/CYRUS-X/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
